...
export default function AdvancedDashboard() {
  ...
  useEffect(() => {
    if (activeTab === "Database" && dbData.length === 0) {
      setIsDbLoading(true);
      // Try to fetch from backend, fallback to mock DB records if Postgres is offline
      axios.get(`${API_URL}/historical/sessions/demo`) // Demo endpoint or real
        .then(res => {
          if (res.data && res.data.length > 0) setDbData(res.data);
          else throw new Error("Empty DB");
        })
        .catch(() => {
          console.warn("DB offline or empty, loading mock historical sessions for UI demonstration.");
          const mockDb = Array.from({ length: 15 }).map((_, i) => ({
            id: `SESS-${10024 + i}`,
            helmet_id: `MN-40${Math.floor(Math.random() * 9)}`,
            start_time: new Date(Date.now() - (i + 1) * 86400000).toLocaleString(),
            duration: `${Math.floor(Math.random() * 4 + 4)}h ${Math.floor(Math.random() * 60)}m`,
            avg_hr: Math.floor(Math.random() * 30 + 70),
            peak_temp: (36.5 + Math.random() * 1.5).toFixed(1),
            events: Math.floor(Math.random() * 5),
            status: Math.random() > 0.8 ? "High Risk" : "Normal"
          }));
          setDbData(mockDb);
        })
      ...
      <div className="flex flex-wrap gap-2 mb-6 border-b border-slate-800 pb-2">
        {TABS.map(tab => (
          <button
            ...
            className={`px-6 py-2 rounded-t-lg font-bold text-sm tracking-wider uppercase transition-all ${
              activeTab === tab 
        ...
        <div className="flex flex-col gap-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="p-8 rounded-2xl border border-slate-800 bg-slate-900/50 backdrop-blur-md flex flex-col justify-center items-center text-center">
              ...
              <div className={`text-4xl lg:text-5xl font-black font-sans hover:font-bold uppercase tracking-widest mb-2 ${
                    prediction === "Normal" ? "text-emerald-400" : 
                  ...
                  {dbData.map((row, idx) => (
                    <tr key={idx} className="hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 text-xs uppercase tracking-wider font-bold rounded border ${
                          row.status === 'Normal' ? 'border-emerald-500/50 text-emerald-400' : 'border-red-500/50 text-red-500'
                        }`}>
                          {row.status}
                        </span>
                      </td>
